...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db_dev/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train  -v 1 $TRAIN_OPTS -i $init_method -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       gmm_train  -v 1 $WORLD_OPTS -i $init_method -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train || exit 1
